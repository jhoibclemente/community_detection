...
#option 3 simple interactive with hover
#img = hvnx.draw(NG, title="Rank based d="+str(delta),with_labels=False, node_color=[NG.nodes[i][coloring] for i in range(n)], pos=nx.fruchterman_reingold_layout(NG))
#hvnx.show(img)

#option 4 (removed isolated nodes)
#img = hvnx.draw(NG, edge_color="grey", with_labels=False, node_size=[v*10 for v in Degree.values()], node_color=[NG.nodes[i]["group"] for i in range(n)], pos=nx.fruchterman_reingold_layout(NG))
#hvnx.show(img)

#option 5 3d
spring_3d_viz(NG,coloring) 
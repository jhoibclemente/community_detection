...
for k in range(6,7):
    ...
    #print(k, "Rand, ARI, F1")
    print(k,', ', rand_score(Predicted, GroundT), ', ',adjusted_rand_score(Predicted, GroundT), ', ',f1_score(Predicted, GroundT, average='micro'))
...
#option 3
img1 = hvnx.draw(G, title="Rank based d="+str(delta),with_labels=False, node_color=[G.nodes[i][coloring] for i in range(n)],  pos=nx.fruchterman_reingold_layout(G))
hvnx.show(img1)

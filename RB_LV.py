...
print("Number of Nodes", len(G.nodes()),len(NG.nodes()))
print("Number of Edges", G.number_of_edges(), NG.number_of_edges())
print("Clustering Coeff:", approximation.average_clustering(G), approximation.average_clustering(NG))
...
#LV = community.louvain_communities(BigC)
LV = community.leiden(BigC)

x = [list(sorted(i)) for i in LV]
print("LV parts:", len(x))
...
#option 2
#nx.draw(NG,with_labels=True,  font_size=5, font_color= 'white',  edge_color="grey", node_size=[v * 10 for v in Degree.values()], node_color=[NG.nodes[i]["gn_grp"] for i in range(n)], pos=nx.fruchterman_reingold_layout(NG))
#plt.show()

#option 3
#img = hvnx.draw(NG, title="Rank based d="+str(delta),with_labels=False, node_color=[NG.nodes[i]["gn_grp"] for i in range(n)], pos=nx.fruchterman_reingold_layout(NG))
#hvnx.show(img)

#option 4 (removed isolated nodes)
#img = hvnx.draw(NG, edge_color="grey", with_labels=False, node_size=[v *2 for v in Degree.values()], node_color=[NG.nodes[i]["group"] for i in range(n)], pos=nx.fruchterman_reingold_layout(NG))
#hvnx.show(img)
